...
`Julia` is so performant, that it's libraries for scientific computing will work with normal variable types - without needing to package up their own types. So solvers from `DifferentialEquations`, or neural networks defined in `Flux` can be immediately combined with the `Julia` probabilistic programming language, `Turing`.
...
* **the mode/direction to apply AD**, `Reverse` 
  Each pass of a reverse-mode AD computes gradients of all inputs w.r.t. a single output (as a vector-Jacobian product). In Forward mode, each pass computes gradients of a single input w.r.t. all outputs (as a Jacobian-vector product). Consequently, there are efficiency trade-offs associated with this selection, depending on the number of inputs and outputs of...
...
* **the function we are differentiating**, `(net, funs, inputs, targets) -> find_loss(net, funs, inputs, targets)[1]`
