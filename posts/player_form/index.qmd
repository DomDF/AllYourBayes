...
delta_xG_plot <- function(matchday_df, label_delta = 1/5, 
  ...
  ggplot(data = matchday_df, mapping = aes(x = match_date, y = delta_xg))+
    ...
    ggrepel::geom_text_repel(mapping = aes(label = opponent),
                             family = 'Atkinson Hyperlegible', size = 5)+
    ...
    geom_text(mapping = aes(x = lubridate::ymd(date_lab_overp), y = y_lab_overp + label_delta, 
              ...
              col = 'forestgreen', family = 'Atkinson Hyperlegible', size = 5)+
    ...
    geom_text(mapping = aes(x = lubridate::ymd(date_lab_underp), y = y_lab_underp - label_delta, 
              ...
              col = 'firebrick', family = 'Atkinson Hyperlegible', size = 5)+
    scale_x_date(breaks = scales::date_breaks(width = 'month'))+
    DomDF::theme_ddf_light(base_family = 'Atkinson Hyperlegible', base_size = 24)+
...
Similarly to the [posts on multi-level models](https://www.allyourbayes.com/post/2020-12-10-uncertainty-in-xg-1/), this will also be split into 2 parts. Part 1 (here) will focus on the features of a Gaussian process that are well suited to approximating player form. Part 2 (in preparation) will include more technical details and more code.
...
n_pp <- 1e2; days_forecast <- 10

alpha_params <- DomDF::lnorm_params(norm_mean = 3, norm_sd = 1)
rho_params <- DomDF::lnorm_params(norm_mean = 8, norm_sd = 2)
...
gp_form_model <- cmdstanr::cmdstan_model(stan_file = 'player_form.stan')

gp_form_fit <- gp_form_model$sample(data = form_model_data, seed = 1008,
...
f_pp_params <- purrr::map_chr(.x = id, .f = function(x) paste0('delta_xG_pred[', x, ']'))
...
EW_GP_sample <- ggplot(data = f_draws |>
         dplyr::filter(Iteration %% 504 == 0, Chain == 2))+
  geom_line(alpha = 1/4,
...
EW_full_GP <- ggplot(data = post_pred_GP_df)+
  stat_density_2d(geom = 'raster', contour = FALSE,
                  mapping = aes(x = days, y = value, fill = after_stat(density)))+
  ...
  geom_line(data = f_draws |>
              dplyr::filter(Iteration %% 150 == 0, Chain == 2),
            alpha = 1/4, col = 'white',
            mapping = aes(x = days, y = value, group = Iteration))+
  scale_x_continuous(name = 'Time through season (days)', breaks = seq(from = 0, to = 120, by = 20))+
...
There may be other sources of information that could help, such as performances in other competitions or even in training. Given we are not sure of the extent that these should inform the model, there is an argument to use a multi-level (partial pooling) structure, as was used to improve [player-specifc xG estimates](https://www.allyourbayes.com/post/2020-12-10-uncertainty-in-xg-1/).
